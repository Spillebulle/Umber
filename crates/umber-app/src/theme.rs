...
pub struct Palette {
    ...
    /// `every_link_group_has_a_colour` pins the two together.
...
impl Palette {
    pub const fn graphite() -> Self {
        Self {
            ...
            // Six hues spread round the wheel and kept off the accent's own
            // warm gold, at a lightness that reads against this surface
            // without competing with the layer name beside it.
            link_colours: [
                Color32::from_rgb(0x5E, 0x9C, 0xD3), // blue
                Color32::from_rgb(0x7F, 0xB0, 0x69), // green
                Color32::from_rgb(0xB0, 0x8C, 0xE0), // violet
                Color32::from_rgb(0x4F, 0xB3, 0xAE), // teal
                Color32::from_rgb(0xD2, 0x78, 0x9B), // rose
                Color32::from_rgb(0xC9, 0xA2, 0x27), // gold
            ],
        }
    }
    ...
    pub const fn paper() -> Self {
        Self {
            ...
            // The same six hues taken dark enough to read as ink on paper.
            link_colours: [
                Color32::from_rgb(0x2E, 0x6D, 0xA4),
                Color32::from_rgb(0x4E, 0x7C, 0x3A),
                Color32::from_rgb(0x7A, 0x55, 0xB0),
                Color32::from_rgb(0x1F, 0x7F, 0x7A),
                Color32::from_rgb(0xA5, 0x44, 0x68),
                Color32::from_rgb(0x8A, 0x6E, 0x10),
            ],
        }
    }
    ...
    ///
    /// The number comes out of a file, so it is taken modulo the table rather
    /// than indexed with — a document written by a build with more groups than
    /// this one has would otherwise be a panic on the drawing path.
    /// `LayerStack::link` refuses to *make* a group this cannot colour.
...
mod tests {
    ...
    /// A link group is told apart from its neighbours by its colour alone, so
    /// the table has to be as long as the model allows groups and no two
    /// entries may be the same. Both themes: a set of colours that collapsed on
    /// paper would make the mark lie for anybody working light.
    #[test]
    fn every_link_group_has_a_colour_of_its_own_in_both_themes() {
        for kind in ThemeKind::ALL {
            ...
            let unique: std::collections::HashSet<_> = p.link_colours.iter().collect();
            assert_eq!(
                unique.len(),
                p.link_colours.len(),
            ...
            assert!(
                !p.link_colours.contains(&p.accent),
                "{kind:?} gives a group the accent, which already means selected"
            );
