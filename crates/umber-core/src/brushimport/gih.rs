...
pub fn dropped_features(bytes: &[u8]) -> Vec<&'static str> {
    ...
    // `gbr` carries the colour across — so a pipe's sequencing is all it drops,
    // and `sequence_losses` is the one list that knows what its rules mean.
    pipe.sequence_losses()
}
...
mod tests {
    ...
    fn a_pipe_whose_cells_are_all_the_same_brush_is_one_stamp_and_loses_nothing() {
        ...
        // same stamp. The sequence is therefore still reported, which is what
        // says the pipe was *not* collapsed onto the plain one — a collapse
        // would leave nothing to report at all.
        //
        // The colour itself is not among the losses any more: a `TipMask`
        // carries a colour plane, so both cells arrive as their author drew
        // them.
        ...
        assert_eq!(pipe.cells.len(), 2);
        assert_eq!(dropped_features(&mixed), [ANIMATION]);
    }
