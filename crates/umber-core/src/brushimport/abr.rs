...
///
/// Deliberately the same four decisions [`super::gbr::to_brush`] makes, for
/// the same reasons: the mask's long side is the size, the mask keeps its own
/// proportions rather than being padded out to a square, the file's spacing is
/// used where there is one, and pressure is left off because the format carries
/// no dynamics. Build-up is measured rather than assumed — a Photoshop stamp is
/// as likely to be a sparse texture as a GIMP one.
pub fn to_brush(brush: AbrBrush) -> (Brush, TipMask) {
    ...
    let parameters = Brush {
        ...
        build_up: crate::tip::stroke_coverage(&tip, spacing).needs_build_up(),
        ..default
    };
    (parameters, tip)
}
...
mod tests {
    ...
    #[test]
    fn a_stamp_keeps_its_proportions_and_takes_its_long_side_as_the_size() {
        let mut file = be16(2);
        file.extend(be16(1));
        file.extend(sampled_v12(2, "Wide", 4, 2, &[255; 8]));
        let read = from_abr(&file).expect("decode");
        let (brush, tip) = to_brush(read.brushes.into_iter().next().expect("one"));
        // Not padded out to a square: a 4x2 stamp stays 4x2 and the dab spreads
        // it over a dab twice as wide as it is tall.
