...
**The brush editor reaches all of it.** Tip, Dynamics, Inputs, Scatter and
Blending: every field a brush has, colour pickup and dab shape and jitter and
airbrush rate included. The samples in the brush list are stamped from each preset's own
...
- The brush editor's **Texture** and **Wet edges** sections. Tip, Dynamics,
  Inputs, Scatter and Blending are built; the other two have no engine behind
  them — paper grain multiplied into dab coverage does not exist — so they are
