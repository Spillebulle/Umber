...
# fields). On X11 that is `x11rb`, which speaks the protocol over the socket
# itself and links no libxcb; on Wayland it is `wl-clipboard-rs` and
# `smithay-clipboard`, both of which reach `libwayland-client.so.0` and nothing
# else — and winit already opens that, so it is already below. `wayland-cursor`
# came along with `smithay-clipboard` and is pure Rust unless its
# `use_system_lib` feature is on, which it is not.
#
...
# explicit MIME type on every call in both directions.
