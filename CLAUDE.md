...
- **The brush editor's sections are Tip, Dynamics, Inputs, Scatter and
  Blending.** The design names six; Texture and Wet edges have no engine behind
  them and are not drawn at all, and Stabiliser is one slider that rides on Tip.
  Blending and Inputs are names of our own: colour pickup needed a home and none
  of the design's is one, and the modulation table is a *list* where Dynamics is
  three pressure curves — Dynamics is pressure, Inputs is everything else.
  Between them they expose every field of `Brush` — adding one means adding a
